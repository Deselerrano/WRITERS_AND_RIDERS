...
void READ_INFO(READER reader) {
		...
		this_thread::sleep_for(chrono::milliseconds(THREAD_LOCAL_RANDOM_INT(1000,3000)));
		cout << "---> Reader " << reader.GET_NAME() << " finish reading book right now" << endl;
		COUNT_OF_READ++;
        	mtx.unlock_shared();
		this_thread::sleep_for(chrono::milliseconds(THREAD_LOCAL_RANDOM_INT(500,4000)));
	}
}
...
int main() {
	...
	WRITTER_1.SET_WRITER("Dostoevskiy",WRITING);
	WRITTER_2.SET_WRITER("Tyrgenyev",WRITING);
